...
# - Adds: summary.links (airbyte/minio/metabase/n8n/zammad/portal)
...
data:
  app.py: |
    ...
    def build_links() -> Dict[str, str]:
      ...
      # Preferred ingress names (your stack conventions)
      candidates = {
      ...
      for key, tries in candidates.items():
        ...
        for ns, name in tries:
          ...
          if host:
            break
        # fallback: first ingress in ns (best-effort) for that app
...
# API does NOT own the portal ingress. UI owns / + /api routing in your UI script.
